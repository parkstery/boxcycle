...
function computeDistanceBetween(a: { lat: number; lng: number }, b: { lat: number; lng: number }): number {
  return getDistanceMeters([a.lng, a.lat], [b.lng, b.lat]);
}
...
function buildRoadSamples(points: CoachElevationPoint[]): RoadSample[] {
...
export function estimateRoadSlope(upcomingPoints: CoachElevationPoint[]): EstimateRoadSlopeResult {
 ...
 */
export function applyRoadElevationModel(points: CoachElevationPoint[]): CoachElevationPoint[] {
  ...
  /**
   * 짧은 도심 코스에서 DEM 잡음이 “좁은 하천” 패턴으로 오인되면
   * `reconstructLocalBridgeDecks` 가 구간마다 직선 데크를 끼워 넣어 차트가 톱니처럼 각이 진다.
  ...
  /** 1차 roadLike·교량 보정 후 잔여 고주파를 한 번 더 거리 창으로 눌러 차트·코칭 공용 곡선을 완만하게 한다. */
  if (outElev.length >= 3 && totalDist > 1e-6) {
    const secondaryHalf = Math.max(18, Math.min((adaptiveWindow / 2) * 0.42, totalDist * 0.065, 100));
    const synth = samples.map((s, i) => ({ ...s, elevation: outElev[i] }));
    const smoothed = buildRoadLikeElevation(synth, secondaryHalf);
    const first = outElev[0];
    const last = outElev[outElev.length - 1];
    for (let i = 1; i < outElev.length - 1; i += 1) {
      outElev[i] = smoothed[i];
    }
    outElev[0] = first;
    outElev[outElev.length - 1] = last;
  }
