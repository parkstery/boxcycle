 ...
 */
export function usePublicationCatalogHub(options: UsePublicationCatalogHubOptions) {
  const {
    configured,
    user,
    routeGeometry,
    ...
    async (courseId: string) => {
      ...
      try {
        if (user && basicActiveHubCourseId && basicActiveHubCourseId !== courseId) {
          await deletePublicationSessionMember(user.uid, basicActiveHubCourseId).catch(() => {
            /* noop */
          });
        }
        const prevOfficial = activeOfficialCourseIdRef?.current;
        if (user && prevOfficial && prevOfficial !== courseId) {
          await deletePublicationSessionMember(user.uid, prevOfficial).catch(() => {
            /* noop */
          });
        }
        let payload = null;
