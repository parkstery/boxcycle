...
export type FirstRideIntroCardProps = {
  /** Publication publicTitle — 입문 경로 제목 */
  introTitle: string;
  /** Publication 제목에서 추출한 목적지(없으면 null) */
  introDestination?: string | null;
 ...
 */
export function FirstRideIntroCard({
  introTitle,
  introDestination = null,
  onStartIntro,
}: FirstRideIntroCardProps) {
  return (
    <div className="first-ride-anchor" aria-label="입문 경로">
      <div className="first-ride__card hud-glass" role="group">
        <p className="first-ride__title">{introTitle}</p>
        <p className="first-ride__body">
          {introDestination ? `${introDestination} 입문 경로` : "입문 경로로 첫 도로를 달려봐요"}
        </p>
