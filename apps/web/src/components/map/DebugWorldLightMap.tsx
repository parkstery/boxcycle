...
export function DebugWorldLightMap({
  ...
  useEffect(() => {
    ...
    const onStyleLoad = () => {
      ...
      console.log("[DebugWorldLight] style-reload", { rebound });
    };
  ...
  useEffect(() => {
    ...
    const syncWithLngLat = (phase: string | null, lngLat: LngLat | null) => {
      ...
      live.once("idle", () => {
        const querySourceCount = live.querySourceFeatures(DEBUG_SRC_ID).length;
        const queryRenderedCount = live.queryRenderedFeatures({ layers: [DEBUG_LAYER_ID] }).length;
    ...
    const tick = async () => {
      ...
      try {
        const { rows } = await fetchPublicPublicationPresencesDetailed();
        if (cancelled) return;
        const first = rows.find((r) => r.representativePoint)?.representativePoint ?? null;
        syncWithLngLat(phase, first ?? PHASE_B_FALLBACK_LNGLAT);
      } catch {
        if (cancelled) return;
        syncWithLngLat(phase, PHASE_B_FALLBACK_LNGLAT);
      }
    };
