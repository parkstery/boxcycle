...
주행 중에는 **「오늘 N km」**(세션) + **「누적 M / 전체 km (P%)」**(경로 위치·진행률)을 함께 보여 준다.
`getByLabel('오늘 거리')`·`getByLabel('누적 진행')`으로 구분한다 — `/` 파싱이나 주행 전 값으로
폴링하면 주행이 시작되기도 전에 목표 거리에 도달한 것으로 오판해 곧바로 종료를 눌러 버린다(실제로 겪은 red).
