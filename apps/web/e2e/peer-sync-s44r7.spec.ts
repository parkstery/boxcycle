...
/**
 * S4-4R7 — R5 와 같은 정렬·밀도. 샷만 전체 뷰 PNG(네임태그 포함). S44R5-* 는 덮지 않는다.
 */
const __dirname = path.dirname(fileURLToPath(import.meta.url));
const OUT_DIR = path.resolve(__dirname, "../../../document/ops/sync-relay");
const SHOT_DIR = path.join(OUT_DIR, "S44R7-shots");
...
test.describe("S4-4R7 PNG pixel capture", () => {
  ...
  test("같은 조건 · PNG 무손실 · 네임태그 포함 샷", async ({ browser }) => {
    ...
    console.log(`[s44r7] aligned=${aligned} gapAtOpen=${gapAtOpen}`);

    fs.mkdirSync(SHOT_DIR, { recursive: true });

    if (!aligned || gapAtOpen == null || Math.abs(gapAtOpen) > 5) {
      fs.writeFileSync(
        path.join(OUT_DIR, "S44R7-summary.json"),
        JSON.stringify(
          {
            instruction: "S4-4R7",
    ...
    console.log(`[s44r7] pngShots=${shots.length} windowMs=${SHOT_WINDOW_MS} first=${shots[0]?.file ?? null}`);
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "S44R7-manifest.json"),
      JSON.stringify(
        {
          instruction: "S4-4R7",
          shotMedianDtMs: shotMedianDt,
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, "S44R7-capture.json"),
      JSON.stringify({ ...summary, alignLog, scoutPeak, judgment: dump.judgment, events: dump.events }, null, 2),
      "utf8",
    );
    fs.writeFileSync(path.join(OUT_DIR, "S44R7-capture-summary.json"), JSON.stringify(summary, null, 2), "utf8");
    ...
    console.log(
      `[s44r7] p1=${p1} p2=${p2} shotMedianDtMs=${shotMedianDt} frameMedianDtMs=${frameMedianDt} peakSepMs=${peakSepMs}`,
    );
  });
});
