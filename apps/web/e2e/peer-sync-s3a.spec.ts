...
/**
 * S3A — 발행 큐 제거 후 재측정.
 * 산출: document/ops/sync-relay/S3A-chain-events.json · S3A-summary.json
 */
...
test.describe('S3A publish queue removed', () => {
  test.setTimeout(180_000)

  test('재측정 · 수용 조건 집계', async ({ browser }) => {
    ...
    fs.writeFileSync(
      path.join(OUT_DIR, 'S3A-chain-events.json'),
      JSON.stringify(
        {
          instruction: 'S3A',
          elapsedMin,
    ...
    const sum = spawnSync(
      process.execPath,
      ['scripts/peer-sync/s3r-summarize.mjs', 'S3A-chain-events.json', 'S3A-summary.json'],
      { cwd: WEB_ROOT, encoding: 'utf8' },
    )
    if (sum.status !== 0) {
      throw new Error(`s3a summarize failed: ${sum.stderr || sum.stdout}`)
    }
