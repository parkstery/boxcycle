...
/**
 * S3B-3 — D-2 before/after. S3B3_PHASE=before|after · S3B3_RUN=1|2|3
 */
const __dirname = path.dirname(fileURLToPath(import.meta.url))
const OUT_DIR = path.resolve(__dirname, '../../../document/ops/sync-relay')
const WEB_ROOT = path.resolve(__dirname, '..')
const GEO_STOP_M = 900
const PHASE = (process.env.S3B3_PHASE || 'after').toLowerCase() === 'before' ? 'before' : 'after'
const RUN = Math.min(3, Math.max(1, Number(process.env.S3B3_RUN || '1')))
const EVENTS_NAME = `S3B3-${PHASE}-run${RUN}-events.json`
...
test.describe('S3B-3 D-2 low-zoom + spectator', () => {
  test.setTimeout(360_000)

  test(`z15 + path A/B (${PHASE} run ${RUN})`, async ({ browser }) => {
    ...
    const cases: Record<string, { start: { a: number; b: number }; end: { a: number; b: number } }> =
      {}
    const pathA: { lowZoomStart?: { a: number; b: number }; restoreStart?: { a: number; b: number }; restoreEnd?: { a: number; b: number } } = {}

    const departStart = await clocks(pageA, pageB)
    await pageA.waitForTimeout(2_000)
    await setSpeedKmh(pageA, 30)

    // Path A — 저줌 15 s 후 z15 복귀 (B 화면)
    ...
    if (PHASE === 'after' && RUN === 3) {
      const sum = spawnSync(process.execPath, ['scripts/peer-sync/s3b3-summarize.mjs'], {
        cwd: WEB_ROOT,
        encoding: 'utf8',
      })
      if (sum.status !== 0) {
        throw new Error(`s3b3 summarize failed: ${sum.stderr || sum.stdout}`)
      }
    }
