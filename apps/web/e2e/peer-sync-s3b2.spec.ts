...
/**
 * S3B-2 — D-1 전/후 측정. S3B2_PHASE=base|post (기본 post).
 * base: S3B2-base-events.json  (pt9 계측만, D-1 미적용)
...
const EVENTS_NAME = PHASE === 'base' ? 'S3B2-base-events.json' : 'S3B2-chain-events.json'
...
test.describe('S3B-2 D-1 applied speed', () => {
  test.setTimeout(300_000)

  test(`z15-depart · z15-cruise (${PHASE})`, async ({ browser }) => {
    ...
    if (PHASE === 'post') {
      const sum = spawnSync(
        process.execPath,
        ['scripts/peer-sync/s3b2-summarize.mjs'],
        { cwd: WEB_ROOT, encoding: 'utf8' },
      )
      if (sum.status !== 0) {
        throw new Error(`s3b2 summarize failed: ${sum.stderr || sum.stdout}`)
      }
    }
