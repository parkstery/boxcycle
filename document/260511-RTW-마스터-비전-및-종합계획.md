...
| 연결 문서 | [문서 지침](260509-BOXCYCLE-문서-생성-및-수정-지침.md), [현재 단계·1차 마일스톤](260509-BOXCYCLE-현재단계-범위-스택-및-1차마일스톤.md), [코스 수명·UGC 품질 정책](260511-코스-수명-UGC-품질-정책.md), [경로 저장 계층화·Frozen Route·Segment](260511-경로저장-계층화-Frozen-Route-Segment.md), [Firestore Rules 일반화 방안](260511-Firestore-Rules-일반화-방안.md), [Phase별 실행 체크리스트](260511-Phase별-실행-체크리스트-Course-Session-Presence.md), [장기 Postgres 참고](260509-아키텍쳐-DB설계.md), [Firestore 스키마 초안](260509-Firestore-컬렉션-스키마-초안.md) |
...
상세 가격·미션·배지·시즌 정책은 (대체된) [기능 추가 계획](260509-기능-추가-계획-제품-및-아키텍처.md)의 미션·배지·시즌 절을 참고하되, **단일 진실은 본 문서**다.

### 3.1 Guest User (비로그인)

| 구분 | 정책 |
|------|------|
| 가능 | 입문 코스 주행 / Public 코스 주행 / 임시 코스 생성 / 공유 링크 사용 |
| 제한 | Challenge 불가 / Event 참가 제한 / Public 등록 불가 / 랭킹 저장 제한 / 코스 생성 시 거리 ≤ 50km |
| 저장 | inactive 기반 자동 삭제 (권장: 마지막 접속 후 7~10일) |
| 저장 위치 | 1차: 서버 임시 저장(접속 키 기반) — LocalStorage/IndexedDB는 캐시 보조 |

게스트 정책은 [코스 수명·UGC 품질 정책 §2.1](260511-코스-수명-UGC-품질-정책.md) 임시화 항목과 일관시킨다.

### 3.2 Free User (로그인)

| 구분 | 정책 |
|------|------|
| 가능 | Public 코스 생성 / 주행 기록 저장 / 기본 랭킹 / 기본 멀티 세션 생성 |
| 제한 | Challenge 일부 제한 / Event 생성 불가 / 생성 개수 제한(권장 1~3개) / 일일 업로드 제한 / 거리 제한 |

### 3.3 Premium User (유료)

| 구분 | 정책 |
|------|------|
| 가능 | 장거리 코스 생성 / 고급 멀티 세션 / 이벤트 생성 / 확장 저장 / AI 추천 / 고급 통계 |
| 보호 | "정복 컬렉션"은 **점수·활동과 무관하게 영구 보존** ([수명 정책 §5](260511-코스-수명-UGC-품질-정책.md)) |

**유료 차별화 원칙:** 표현 품질("HD 화질") 대신 **서버 자원 소비량**(대규모 세션·장거리 경로·고빈도 동기화·클라우드 저장) 중심. 자세한 무료/유료 표현 가이드는 [기능 추가 계획 §7](260509-기능-추가-계획-제품-및-아키텍처.md) 참조.
...
| `courses` | 정적 코스(지도·경로) + 수명 메타 | [Firestore 스키마](260509-Firestore-컬렉션-스키마-초안.md) §3.5, [수명 정책](260511-코스-수명-UGC-품질-정책.md) §7 |
| `sessions` | 실시간 멀티플레이 방(코스에 연결) | [Firestore 스키마](260509-Firestore-컬렉션-스키마-초안.md) §3.6 |
...
| # | 질문 | 결정 시기 |
|---|------|-----------|
| Q1 | 외부 노출 명칭을 BOXCYCLE 유지할지 Ride The World로 전환할지(앱 스토어·도메인) | Phase 2~3 사이, 마케팅 결정 시점 |
