...
| 연결 문서 | [문서 생성·수정 지침](260509-BOXCYCLE-문서-생성-및-수정-지침.md), [**RTW 마스터 비전 및 종합계획**](260511-RTW-마스터-비전-및-종합계획.md), [Mapbox 시뮬 검증 기록](archive/260508-개발중간보고-HTML과-JS-프로토타입.md), [아키텍쳐·DB 장기안](archive/260509-아키텍쳐-DB설계.md), [Firestore→Postgres 피하기](archive/260509-Firestore-Postgres-이전-체크리스트.md), [실행·리팩터링](archive/260509-app-js-프론트백엔드-분리-1차리팩터링.md), [Phase별 실행 체크리스트](archive/260511-Phase별-실행-체크리스트-Course-Session-Presence.md), [제품 용어 Trailhead·Trail](260517-제품-용어-Trailhead-Trail.md), [사용자 tier·진입](260519-사용자-tier-및-진입-정책.md), [World Activity Presence](260523-World-Activity-Presence-설계.md), [Firestore 스키마 초안](archive/260509-Firestore-컬렉션-스키마-초안.md) |
...
- **Mapbox 기반 실내 사이클링 시뮬 검증**은 완료되었다. 당시 정적 HTML·`app.js` POC는 **[260508 기록](archive/260508-개발중간보고-HTML과-JS-프로토타입.md)** 및 커밋 이력으로만 참고하고, **저장소 루트 레거시 파일은 제거**하였다.
...
| E | 재현 가능 | 스테이징·Hosting URL에서 PM이 계정 2개 데모를 재현한다. | ⬜ **PM 확인** — 배포 URL·시나리오 체크리스트: [수동 스모크](archive/260516-수동-스모크-체크리스트.md) |
...
상세 로드맵은 [260509-아키텍쳐-DB설계.md](archive/260509-아키텍쳐-DB설계.md)의 장기안을 따르되, **현재 단계의 진실은 본 문서**이다.
...
- **이후** 트래픽·쿼리·비용·도메인 요구에 따라 **PostgreSQL 등으로 분리**한다. 분리 시 스키마·데이터 이전 난이도를 줄이려면 [260509-Firestore-Postgres-이전-체크리스트.md](archive/260509-Firestore-Postgres-이전-체크리스트.md)를 설계 시점부터 준수한다.

### 3.3 Mapbox·외부 API

- 과거 검증 POC(260508 기록)는 클라이언트에서 Mapbox를 직접 호출했다.  
- 본 개발에서는 **토큰 보호·레이트 제한**을 위해 **서버 측 프록시(Cloud Functions `getMapboxDirections` 등)** 를 쓴다. 실행 순서는 [260509-app-js-프론트백엔드-분리-1차리팩터링.md](archive/260509-app-js-프론트백엔드-분리-1차리팩터링.md)와 정렬한다.
...
| Route Dock·지명 검색 UI | ✅ | 좌하단 Route Dock, HUD 「지명」 — [UX IA](archive/260515-ux-주행-여정-및-패널-IA.md) |
| Activity World LOD | ✅ | [LOD 설계](archive/260517-Activity-World-지도-LOD-설계.md) |
| Firestore 부하 1차 저감 | ✅ | [(cycle) 종합보고](archive/260515-(cycle)Firestore-부하-경감-조치-종합보고서.md) |
| App 도메인·훅 분리 | ✅ 1차 | [결과 보고](archive/260516-App-도메인-훅-분리-결과-보고서.md) |
...
| RTW `sessions/` · `presence/` | ⬜ | [Phase 1-B](archive/260511-Phase별-실행-체크리스트-Course-Session-Presence.md) |

### 4.4 다음 2주 (우선순위)

1. **1차 마일스톤 E** — Hosting/스테이징에서 2계정 Trail·입문 허브 동행 데모 PM 서명 ([스모크](archive/260516-수동-스모크-체크리스트.md)).
2. **Phase 1-A-3** — Rules 회귀 T1~T3 수동 확인.
3. **World Presence** — 설계 M 잔여·비용 체크리스트와 정합 ([Firebase 비용](260523-Firebase-비용-운영-체크리스트.md)).

### 4.5 과거 POC

루트 정적 `index.html` / `app.js` 제거. 상세는 [260508 기록](archive/260508-개발중간보고-HTML과-JS-프로토타입.md).
