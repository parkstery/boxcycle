...
**최신 입력 안내(2026-09-14 기록 갱신):** `natural_joint_skin/roadcyclist_natural_joint_skin.blend`·`.glb`의 전달 기록을 갱신했다. 제작 보고상 **52,554 tris / 210개 메시**, 전완 축소와 연속 팔꿈치·무릎 피부의 제작/정지 검증 완료다. 정확한 경로·전달 해시·새 객체 대응과 `Guide_*` 그룹은 [최신 라이더 인계 조건](260912-Modelling-최신라이더-인계조건.md) §1에 보존한다. 목둘레 수정은 이번 인계의 완료 항목에 없어 후속 확인 대상으로 남긴다. 새 후보의 사용자 외형 승인·앱 운동 검증은 미완료다. 이번에는 기록만 갱신하며 모델 처리·제품 교체를 실행하지 않는다. 이전 appearance_preserved_lod와 후보 `20260913-162030-d32f2fc2` 및 §1/§7의 구형 입력은 이력으로 보존한다. **2만~3만 tris는 절대 목표가 아니며 외형·접촉 보존을 우선**한다.

**동작 연결 보완:** 최신 몸통/양팔·연속 무릎 피부는 여러 관절에 걸친 메시다. 아래 강체 분할 우선안은 이 새 구조에 그대로 확정 적용하지 않는다. 제작 측의 뼈·혼합 가중치 요구와 현행 Mapbox 명명 노드 회전 방식의 호환성을 먼저 검토하고, 자연스러운 피부 연결이 유지되는 구현 경로와 운동 QA를 확정해야 한다. 가이드 그룹만으로 앱 페달링이 동작한다고 판단하지 않는다.
...
실행 시 [rider-cycle-fit 스킬](../.agents/skills/rider-cycle-fit/SKILL.md)의 **“단계별 승인: 각 단계(0/A/B/C/D/E) 그림 승인 후 다음”**, **“사용자 승인 전 페달 애니메이션으로 넘어가지 않는다”**를 적용한다. 그림·수치·후보 파일을 먼저 준비한 뒤 해당 단계 승인을 받는다. 작업 진행·연결 요청은 특정 후보 그림의 승인으로 기록하지 않는다.
...
원본 GLB의 구조·해시·정점 크기와 제작 작업의 최종 결과는 확인했다. 2026-09-11 `.blend`를 직접 열어 **207개 메시, armature/vertex group/action 모두 없음**을 확인했다. 본 가중치에 의한 기존 분해기를 직접 재사용할 수 없으며, A 단계에서 명시적 메시→부위 매핑을 만든다. 실제 정적 접점 오차, 8위상 변형 품질, 목표 기기의 성능은 아직 검증하지 않았다.

최신 입력의 단계 0 후보는 `20260913-162030-d32f2fc2`이며 원본/사본 48개 해시와 Blender 렌더 4장을 검증했다. 상세 근거는 [외형 보존 후보 단계 0 기록](archive/260913-Modelling-라이더-교체-단계0-외형보존후보.md)을 따른다. 이전 `20260911-184054-afc08cbf`(34개 해시, 현행 제품 자세 검사 13/13 PASS)는 [9월 11일 기준선](archive/260911-Modelling-라이더-교체-단계0-기준선.md)의 이력이며 최신 후보의 승인 근거가 아니다.
