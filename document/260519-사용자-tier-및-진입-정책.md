...
| **D2** | `!user` 시 주행·경로·presence **하드 차단** · local-only 신규 쓰기 제거 | hooks | **부분** — UI `gate`·Firestore `auth` 필수, 로컬 영속 신규 쓰기 감사 필요 |
| **D3** | `users.tier` 필드 · 온보딩/닉네임/연동 CF에서 설정 | Functions | 대기 |
| **D4** | Rules: `tier` 클라 쓰기 금지 · 기능별 read 조건 | `firestore.rules` | 대기 |
