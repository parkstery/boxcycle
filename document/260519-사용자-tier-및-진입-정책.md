...
| 작성일 | 2026-05-19 |
| 상태 | **부분 구현(2026-05-19)** — D1~D6 1차 완료. Stripe Secrets·배포·이벤트 UI는 후속 |
| 상위 | [RTW 마스터 비전](260511-RTW-마스터-비전-및-종합계획.md), [Route Token 경제](260518-Route-Token-경제-설계.md) |
| 연결 | [로그인·인증 흐름](archive/260515-로그인-인증-코드-위치-및-흐름-보고서.md), [Firestore 스키마 초안](archive/260509-Firestore-컬렉션-스키마-초안.md), [코스 수명·UGC](archive/260511-코스-수명-UGC-품질-정책.md) |
...
flowchart TB
  subgraph forbidden ["서비스 밖"]
    N["user === null · 인증 세션 없음"]
  end
  subgraph allowed ["서비스 안"]
    G["anonymous · Guest · 익명 인증"]
...
- TTL·저장 정책: [코스 수명 §2.1](archive/260511-코스-수명-UGC-품질-정책.md) Guest 행
...
- `user === null` 인 채 지도·주행·Firestore 읽기 — **제거됨(2026-05)** · 로그아웃 후에도 `AuthGateCard`(`gate`)로 기능 차단
...
| `user === null` | 로그아웃 후 **맵만 보기**(`postSignoutMapSession` 등) | **기능 불가** · `gate` + 「다시 시작」으로만 재진입 |
| 진입 | 사용자가 「게스트」**수동 클릭** | **자동 익명** (`useAppAuth` 부트) · 실패 시 「다시 시도」 |
...
| 단계 | 작업 | 산출 | 상태 |
|------|------|------|------|
| **D0** | 본 문서 채택 · [RTW §3](260511-RTW-마스터-비전-및-종합계획.md) 용어 수정 | 문서 정합 | **완료** |
| **D1** | GuestEntry 동의 후 익명 자동 · `VITE_ALLOW_UNAUTH_MAP` QA · `postSignoutMapSession` 제거 | `GuestEntryCard` · `useAppAuth` · `App.tsx` | **완료** |
| **D2** | `!user` 시 주행·경로·presence **하드 차단** · local-only 신규 쓰기 제거 | hooks | **완료** — `clientPersistencePolicy`·게이트·지도 핀 차단 |
| **D3** | `users.tier` 필드 · 온보딩/닉네임/연동 CF에서 설정 | Functions | **완료(1차)** — `ensureAnonymousUserTier`·닉네임→`registered_free` |
| **D4** | Rules: `tier` 클라 쓰기 금지 · 기능별 read 조건 | `firestore.rules` | **완료(1차)** — tier·subscription 클라 보호 |
| **D5** | quota 표(경로 생성·이벤트) + CF 검증 | [tier quota 정책](260519-tier-quota-정책.md) | **완료(1차)** |
| **D6** | 결제·`registered_paid` | [tier subscription 정책](260519-tier-subscription-정책.md) | **완료(1차)** |
...
| 2026-05-19 | Mermaid — 다크 모드용 투명 노드·밝은 텍스트(`themeVariables`) |
| 2026-05-19 | 코드 — `users.tier`, GuestEntry, `useUserTier`, Rules 퍼블릭 신청, `mergeUserAuthMeta`, local 영속 차단 |
| 2026-05-19 | §5·§6 — D0/D1 완료 반영, `postSignoutMapSession` → `userSignedOut`·gate |
| 2026-05-19 | D2–D4 1차 구현 — `clientPersistencePolicy`, `users.tier`, Rules |
| 2026-05-19 | D5 — [tier quota 정책](260519-tier-quota-정책.md), `assertTierQuotaHttp`, onCreate guard |
| 2026-05-19 | D6 — [tier subscription 정책](260519-tier-subscription-정책.md), Stripe Checkout·Webhook |
