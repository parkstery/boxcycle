...
> **1차 원인: `map.isStyleLoaded()`를 렌더 가드로 사용한 것.** 위성 + 3D terrain 스타일에서 (그리고 mapbox-gl 버전이 올라가면서) 이 값이 **영구히 `false`** 를 반환했고, 그 결과 Activity World dot 레이어를 만드는 `syncActivityWorldDotLayers`가 매번 즉시 `return` 되어 **dot 레이어 자체가 생성되지 않았다.** 가드를 제거(`if (!map.style) return` + `try/catch` + `style.load`/`idle` 재시도)하자 해결되었다.
>
> **2차 원인(최신 코드 main): publication-only 데이터 경로.** `main`은 `courseActivity`/`liveCourseRides` 대신 publication presence "전용" 모드라 실제 주행을 누락시켰다. `courseActivity` catalog 방식으로 되돌려 최종 해결(§6.1).
...
## 6.1 (후속) 최신 코드(main)에 이식하며 발견한 추가 원인

`isStyleLoaded` 가드 제거를 `main` 계열(`fix/red-dot-render`)에 이식한 뒤, **렌더는 됐지만 실제 주행이 dot으로 안 잡히는** 2차 문제가 드러났다.

### 6.1.a publication-only 데이터 경로 (실제 주행 누락)
- `main`은 5/23 이후 `useAppMapOverlays`에서 **`publicationPresenceWorldMapEnabled = true`(publication presence 전용)** 로 동작했다.
- 이 모드는 `activityWorldRaw`를 **publication presence dots만**으로 채우고, `courseActivity` catalog(`catalogOverlayEnabled=false`)와 `liveCourseRides`(`worldLiveCourseRideOverlayEnabled=false`)를 **둘 다 끈다.**
- publication presence 경로는 실제 주행을 World dot으로 잡지 못해(`pubFetch 0`) **서울/도쿄 주행이 안 보였다.**
- **수정:** `publicationPresenceWorldMapEnabled = false` 로 비활성화 → `courseActivity` catalog + `liveCourseRides` 경로(5/19 검증 방식) 복원. 주행 dot 정상 표시 확인.
- publication presence 는 트래픽 절감 목적의 미완성 경로였으며, 추후 재설계 시 다시 켤 수 있다.

### 6.1.b 신규 주행의 실시간 반영 지연 (경미)
- user A가 맵을 보는 중 user B가 **새로** 주행을 시작하면, dot 표시까지 **최대 ~90초** 지연(또는 새로고침 시 즉시).
- 원인: 활성 코스 목록 갱신(`WORLD_PRESENCE_POLL_MS = 90s`) → 그 목록 기반 `courseActivity` 조회의 2단계 폴링.
- 버그가 아니라 비용 절감용 폴링 주기. 개선하려면 폴링 단축 또는 활성 코스 목록 `onSnapshot` 실시간 구독. (우선순위 낮음)
