...
| [260903-동행 인원수 갱신 지연 4C 작업지시서](ops/route-relay/260903-동행-인원수-갱신지연-4C-작업지시서.md) | **현재 작업 `HUD-COMPANION-LATENCY-4C`** — 「지금 N명」이 60초 폴 위상 차로 두 화면이 갈라짐. 표시값 `max(aggregate, 1+실시간 다른 라이더)`. TTL·주기 수치는 유지 | 코드 반영 · 2창 확인 대기 |
