...
| "이 모듈이 **어느 도메인**인가·무엇을 import 해도 되나" | [lib 도메인 경계와 의존 방향](260925-RTW-lib-도메인-경계와-의존-방향.md) |
| "구조를 **바꾸려면**·게이트를 **세우려면**" | [구조 게이트 운용 지침](260926-RTW-구조-게이트-운용-지침.md) |
...
| [260924-RTW-UI-공간밀도-원칙](260924-RTW-UI-공간밀도-원칙.md) | 화면 공간 사용 규칙(D1~D7) — 폰 가로 275px, 신규·수정 UI 전부에 자동 적용 | SoT |
| [260714-RTW-Ontology](260714-RTW-Ontology.md) | 용어·개념·관계·금지어 — "X가 무엇인가"의 답 | SoT |
| [260926-RTW-구조-게이트-운용-지침](260926-RTW-구조-게이트-운용-지침.md) | 구조 도구 사용법 — dep-layers 선언 → codemod 이동 → 게이트 확인. 게이트 규율 4가지(깨뜨린다·M0·고치면 죽는 검산 금지·규칙 완화 가시화), `tsc` 가 못 잡는 5종 | SoT |
| [260925-RTW-lib-도메인-경계와-의존-방향](260925-RTW-lib-도메인-경계와-의존-방향.md) | `lib/` 도메인 책임·허용 의존 DAG — 제품 개념 관계와 코드 import 방향의 분리. 기계 판독본 `apps/web/dep-layers.json` | SoT |
...
- **[라이더 신·구 모델 교체 완료 보고](260922-RTW-라이더-신구모델-교체-완료보고.md)** — 2026-09-21 교체 완료(커밋 `d386b78`). 최종 탑재 폴더·파일명·해시, 앱 코드 맵(`config.ts` / `preservedRiderLayer.ts` / `preservedRiderRig.ts` / `MapView.tsx`), 신·구 스펙 실측 비교, 문제·해결 9건, 재교체 체크리스트·불변식. **모델 재교체 전 필독.**
- [라이더 형상 보존 교체 계획](260920-RTW-라이더-형상보존-교체계획.md) — 중단된 교체의 상태 대조와 원본 보존 경로 검증 계획. 2026-09-21 집행 완료 → 결과는 위 완료 보고 참조.

UI/UX 작업 인수인계: [260917-UI-UX-개선-작업-인수인계](260917-UI-UX-개선-작업-인수인계.md) — 새 세션이 맨땅에서 화면 작업을 이어받기 위한 브리핑(불변 규칙·최근 변경 표면·열린 과제·검증 명령·실제로 데인 함정). 착수 대상 미정. 이전 판 260915 는 archive.

진행 중 실행 계획: [Modelling 라이더 교체](260910-Modelling-라이더-교체-실행계획.md) — natural_joint_skin 인계 기록 갱신, 목둘레 완료 미확인·외형 승인·앱 운동 검증 미완료. [최신 라이더 인계 조건](260912-Modelling-최신라이더-인계조건.md) — 파일·해시·연속 피부 구조와 다음 단계. 이전 단계 0은 [외형 보존 후보 이력](archive/260913-Modelling-라이더-교체-단계0-외형보존후보.md)으로 보존. **제품 교체는 2026-09-21 완료**(후보 `20260921-041426-cd81398e`, `preserved` 모드) → [완료 보고](260922-RTW-라이더-신구모델-교체-완료보고.md).
...
| [260903-동행 Trail 단일 진실 · 이탈 반영 4D 작업지시서](ops/route-relay/260903-동행-Trail단일진실-이탈반영-4D-작업지시서.md) | **현재 작업 `HUD-COMPANION-TRAIL-TRUTH-4D`** — 동행=같은 Trail 만. 인원수는 Trail 실시간. aggregate 인원수 절 폐기(heat 유지). 이탈 즉시 감소·접속 목록 낙관 정리 | 코드 반영 · 2창 확인 대기 |
| [260903-동행 인원수 갱신 지연 4C 작업지시서](ops/route-relay/260903-동행-인원수-갱신지연-4C-작업지시서.md) | **`HUD-COMPANION-LATENCY-4C`** — 합류 즉시 반영 검증 완료. `max(aggregate, 실시간)` 은 4D 가 Trail 단일 진실로 뒤집음 | **합류 PASS** · 이탈은 4D |
| [260903-giant 20배 제품 채택 G-3 작업지시서](ops/giant-relay/260903-giant-20배-제품채택-G3-작업지시서.md) | **현재 작업 `GIANT-ADOPT-20X-G3`** — 2026-08-27 「자이언트 미채택」을 **20배 한정으로 뒤집는다**. 카메라 전환점 36배 아래라 공식 미영향. 400배는 미채택 유지 | 실행 대기 |
| [260903-동행 HUD 「다른 라이더 없음」 오판정 4B 작업지시서](ops/route-relay/260903-동행HUD-다른라이더없음-오판정-4B-작업지시서.md) | **`HUD-COMPANION-CONSISTENCY-4B`** — 「지금 2명 주행」과 「다른 라이더 없음」 동시 표시. `7cbc007` 회수. 빈 문장은 live ride 행 기준 | **회수 완료** · 사용자 화면 확인 |
| [260905-화면 정리 — 센서 칩을 RouteDock 으로 6A 작업지시서](ops/route-relay/260905-화면정리-센서칩-RouteDock-6A-작업지시서.md) | **현재 작업 `UI-DECLUTTER-SENSOR-6A`** — 화면 표면 8덩어리 중 우상단 정리. 센서 칩을 Go 옆(RouteDock)으로 옮겨 「경로→센서→Go」를 한 시선에 | 실행 대기 |
| [260904-자동 Route 중복 구간·정복 손실 5A 작업지시서](ops/route-relay/260904-자동Route-중복구간-정복손실-5A-작업지시서.md) | **현재 작업 `AUTOROUTE-OVERLAP-5A`** — 거리를 맞추려 중복 구간을 만든다(Chief 실사용). 중복은 정복이 안 되어 토큰·거리 대비 새 도로가 손실된다. 원인(우회 vs provider) 계측 후 후보 선택에 중복 페널티 | 실행 대기 |
...
| [260913-도로망-다음접속-2단계-검수PASS-후속-작업계획](ops/ride-relay/260913-도로망-다음접속-2단계-검수PASS-후속-작업계획.md) | 2단계 Phase A/B | 완료 |
| [260913-도로망-다음접속-2단계-완료보고](archive/260913-도로망-다음접속-2단계-완료보고.md) | Hosting 스모크·병합 기록 | archive |
| [260913-도로망-주행중-3단계-작업지시서](ops/ride-relay/260913-도로망-주행중-3단계-작업지시서.md) | `RIDE-IN-RIDE-HIERARCHY-3` · PR #17 | 완료·Hosting |
| [260913-도로망-주행중-3단계-재작업-시도1](ops/ride-relay/260913-도로망-주행중-3단계-재작업-시도1.md) | H1/H2/H3 보완 | 종결 |
| [260913-도로망-주행중-3단계-완료보고](archive/260913-도로망-주행중-3단계-완료보고.md) | 병합 `5beb1a2` · Hosting | archive |
| [260913-지도-현재위치표시-작업지시서](ops/map-relay/260913-지도-현재위치표시-작업지시서.md) | **현재 `MAP-SELF-LOCATION-MARKER-1`** · `260913-개발팀장` | 활성 |
| [260913-개발팀장-착수브리핑-현재위치표시](ops/map-relay/260913-개발팀장-착수브리핑-현재위치표시.md) | 현재 위치 Marker 착수 | 활성 |
| [260919-표고 API 한도 소진 재발 방지 작업지시서](ops/ride-relay/260919-표고-API-한도-소진-재발방지-작업지시서.md) | **현재 `RIDE-ELEVATION-QUOTA-1`** — Open-Meteo 일일 한도 소진(429)이 회귀로 오인됐다. e2e 전역 스텁·routeSig 캐시·429 전용 메시지 | 미착수 |
| [260913-도로망-재방문실험-4단계-작업지시서](ops/ride-relay/260913-도로망-재방문실험-4단계-작업지시서.md) | `RIDE-RETURN-EXPERIMENT-4` | **폐기**(별도 태스크) |
| [260913-3단계잔여-통계-마지막일일-작업계획서](ops/ride-relay/260913-3단계잔여-통계-마지막일일-작업계획서.md) | **현재 `RIDE-3R-STATS-LAST-DAILY`** · PR #19 검수 PASS | Chief 병합·배포 대기 |
| [260913-개발팀장-착수브리핑-3R통계](ops/ride-relay/260913-개발팀장-착수브리핑-3R통계.md) | `260910-개발팀장` 착수 | 종결(시도1 PASS) |
...
| [260919-RTW-라이더-GLB-교체-작업계획](260919-RTW-라이더-GLB-교체-작업계획.md) | 제품 라이더 GLB 교체 SoT (rev.3) — 사전점검 P/P2/P3 반영, 단계 0/A/B/C/D/E | 진행중 |
| [출시 전 확인사항](<출시 전 확인사항.md>) | 개발 완화 수치 복원 목록 | 반영중 |
...
| 문서 | 역할 |
|------|------|
| [260926-RTW-Phase5-lib-도메인-분할-완료보고](archive/260926-RTW-Phase5-lib-도메인-분할-완료보고.md) | Phase 5 — `lib/` 평면 0·순환 0·pre-push 게이트 7종. **조용히 깨져 있던 것 5건**(셀 ID 3중 복제·동행 페달 계단화·레이어 top 다툼·red 계약 시험 방치·에뮬레이터 지도 블랙아웃). 「순환은 폴더 이동이 끊어주지 않는다」·「고치면 죽는 검산은 검산이 아니다」. 남은 위반 35건의 성격별 분류 |
| [260922-RTW-개발중간점검-보고서](archive/260922-RTW-개발중간점검-보고서.md) | 2026-09-22 중간 점검 — 제품 개요·영역 진전·`main`/`main2`·Vercel/Firebase 분기·P0~P2 |
| [260827-라이더-자이언트-스케일-실험-종결](archive/260827-라이더-자이언트-스케일-실험-종결.md) | G-1 20배·G-2 400배 **미채택**. `main2` 병합 금지. 보존 위치 `260825-giant` · 태그 `experiment/260825-giant` |
...
| 2026-09-22 | 라이더 신·구 모델 교체 완료 보고 등재 — [260922](260922-RTW-라이더-신구모델-교체-완료보고.md). 260920 계획서 상태·Modelling 실행계획의 「제품 교체 미실행」 문구 갱신 |
| 2026-09-22 | 개발 중간 점검 보고서 등재 — [archive/260922](archive/260922-RTW-개발중간점검-보고서.md) |
