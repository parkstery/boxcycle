...
| [260903-동행 HUD 「다른 라이더 없음」 오판정 4B 작업지시서](ops/route-relay/260903-동행HUD-다른라이더없음-오판정-4B-작업지시서.md) | **현재 작업 `HUD-COMPANION-CONSISTENCY-4B`** — 「지금 2명 주행」과 「다른 라이더 없음」이 동시 표시. 원인은 접속·동행 중복 제거 필터가 동행 목록을 비우는 것. 미병합 `fix/hud-companion-consistency` 의 수정을 회수 | 실행 대기 |
