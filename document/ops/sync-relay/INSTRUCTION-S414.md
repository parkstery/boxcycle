...
- 처음 이상해지는 단계 (C5): 당시 보고는 ⑤ peer DOM transform. **S4-15 §0 에서 「peer DOM 원인 확정」·「카메라·self 무죄」는 철회.** 확정은 문제 구간 = displayDistM 이후 ~ 최종 DOM.
- 반례 고정 (C6): `S414-pre-fix-fail.json`. displayDistM 불변식 **통과**. relAnchor 층 **실패**. 이 스크립트는 **실패 trace** 이지 회귀 replay 가 아니다.
- 계층 확장 (C7): 기존 displayDistM 검사만으로는 통과 → peerAnchor−selfAnchor DOM translate 재검사.
- NEXT: 그 구간을 셋으로 가른다 (S4-15).
