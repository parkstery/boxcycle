...
## 6. 결과 — S4-M1 (2026-08-14) · **§2-1에서 정지** (구현·after·회귀·커밋 없음)

탭이 산 동안 주행을 끝내거나 Trail을 바꿔도, 늦게 도착한 motion 쓰기가 **이미 지운 위치를 다시 그려** 동행 화면에 유령이 남는다. 같은 Trail에 바로 복귀한 경우는 이 라운드에서 반례를 못 잡았다.

S4-M0 커밋 파일 5개 · `REPORT.md` HEAD 블롭 sha256 `cbf1554…` — S4-M1R §0 에서 일치 확인.

### §2-1 반례 표 (`S4M1-lifecycle-baseline.json`, elapsed 3.3 min)

| | 결과 | 관측 |
|---|---|---|
| M1 | FAIL (반례) | `existsAfterSettle=true` 7/7 |
| M2 | FAIL | 숨김·종료 둘 다 노드 잔존 |
| M3 | FAIL (반례) | `previousTrailResurrected=true` 6/6 |
| M4 | PASS (반례 실패) | `newSessionNodeKept=true` 6/6 — 9초 뒤 400ms 6회 샘플 |
| M5 | FAIL | `pt3_ok0=0` · `motionErrorEvents=[]` |
| M6 | FAIL | `w2=null` |

M1~M6 after · W-1 · W-2 · §2-5 회귀 · 3커밋 — **미실시.**

**정지 이유(당시):** M4 경쟁이 deferred 없이 성립하지 않는다고 보고 멈춤. **S4-M1R §0 정정:** 과대 결론. 복귀 후 새 세션에도 전역 지연이 남아 삭제가 빈 경로를 지웠고, 관측도 5Hz 재생성을 못 본다.

실패·미완: 구현 전체. 이견: M4 비성립 결론은 감리가 정정함.
