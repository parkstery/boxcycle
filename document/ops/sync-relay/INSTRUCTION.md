...
- **브랜치**: `fix/multiplayer-read-amplification` · worktree `C:/20.HDev/rtw-sync-s4-2/repo` (현재 `3dfa1f3`)
...
[S4-5 결과]
- 축 교체 요약 (T0): `span`·`t` 를 `serverAtMs` 로 계산. `offset = EMA(recvAtMs − serverAtMs)`, `renderTime = (now − offset) − 160`. 상수 160·16·1200 불변.
- 시계 폴백 횟수 (T1): jitter 시나리오 **0**. `serverAtMs=0` 가드 시험에서 폴백 ≥1 을 확인. 제품 ingest 는 rtdb-only.
- dedup·stall 불변 (T2·T3): distM 전진 dedup 유지. stall 은 `entity.speedMps`. 정지 오버슛 시험 0 m. 홀드 ≤ 1200 ms.
- 지터 시나리오: 수정 전 fail 내용 → `recv-jitter 구간속도 6.67km/h (송신 5.00km/h 대비 rel=0.333 > 0.15) @ t=5020ms` (100 ms 구간 게이트; 10 ms 순시는 10.00 km/h rel=1.000 @ t=3190ms). 수정 후 **pass**. 로그 `S45-jitter-before.json` · `S45-jitter-after.json`.
- gap_px 전/후 (T5): 대리 29.2 px/m (R7 창 94 px / 3.22 m. 지도 투영 아님. 합격선 없음). 전 반전 69 · 최대|Δ| 0.406 · 진폭 2.028 → 후 211 · 0.256 · 0.377.
- 픽셀 재측정: R7 진폭 10.8px → **6.06 px** (T6). 반전 16→25 · 최대|Δ| 6.4→4.29. S0 PASS. gap 3.84–4.14 m · 5 km/h · leftFlat · PNG 40장. R7 산출물 미덮음.
- 예측 대조 (T7): 6~7px 로 줄었는가 — **진폭은 줄었다 (10.8→6.06, 예측 구간에 들어옴). 반전 횟수는 늘었다 (16→25).**
- 1단계·S4 비용 무훼손 (T8·T9): `test:peer-s3a-replay` 기존 10 시나리오 ✓ · d0·d1 pass · s42 15 · s43 11 · `tsc -b` · eslint 0.
- 해석: 도착 지터가 기여분이었는가 — 진폭만 보면 예측과 맞다. 반전이 같이 줄지 않았고 창 gap 이 R7(3.06–3.48)과 완전 동일하지 않다. **주원인이라고 단정하지 않는다.** S4-4 미해결. ④⑤ 보류.
- NEXT: 감리. ④ pose 통합 · ⑤ merge 제거는 이번 결과 보고 후 판단.

### T0~T9

| | 항목 | 결과 |
|---|---|---|
| T0 | 축 교체 | **PASS.** span·t = serverAtMs · offset EMA |
| T1 | 시계 가드 | **PASS.** 0·비단조 → recv 폴백. jitter 경로 0회 |
| T2 | dedup 불변 | **PASS.** distM 전진 기준 유지 |
| T3 | stall 불변 | **PASS.** entity.speedMps · 홀드 1200 · 정지 오버슛 0 |
| T4 | 지터 시나리오 | **PASS.** 수정 전 fail (rel=0.333) → 수정 후 pass |
| T5 | gap_px 게이트 | **기록.** 대리 29.2 px/m. 전 2.028 → 후 0.377. 합격선 없음 |
| T6 | 픽셀 재측정 | S0 PASS. peer 25 · 4.29 · **6.06**. R7 16 · 6.4 · 10.8 |
| T7 | 예측 대조 | 진폭은 6~7 구간. 반전은 증가. 기여 가능 · 주원인 미단정 |
| T8 | 1 단계 무훼손 | **PASS.** 기존 시나리오 · d0 · d1 |
| T9 | S4 비용 무훼손 | **PASS.** s42 15 · s43 11 · tsc · eslint 0 |
