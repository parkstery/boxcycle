...
```
U-1R   1 m 구도에서 머리(헬멧 top) 잘림 — Chief 문제 제기 취소로 종결. INSTRUCTION-U1R-보류.md
       Chief 실사용 판정으로 추가 코드 수정 없이 종결
       (당시 계측 bbox 높이 0 무효·GLB AABB 투영 과제는 이력으로만 남긴다)
U-6    구도 미세조정 — Chief 문제 제기 취소로 종결. INSTRUCTION-U6-미실행.md
       Chief 실사용 판정으로 추가 코드 수정 없이 종결
