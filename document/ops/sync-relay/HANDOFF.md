...
## 7. U 계열 — 라이더 카메라·화면 (별개 작업선, 2026-08-14~15 종결)

같은 릴레이에서 진행했지만 **Sync 와 다른 작업선**이다. 브랜치 `fix/map-render-tick`.

### 7-1. ✅ 종결 — 라이더 전후 진동 (U-10, 사용자 육안 확정)

증상은 처음 「화면이 톡톡 튄다」로 보고됐으나, 사용자 재관찰로 **「맵은 가만히 있고 라이더와
네임태그만 전후로 진동」**으로 재정의됐다. 원인은 **두 개**였고 둘 다 카메라에 있었다.

```
원인 ①  위치 지연이 프레임 간격에 따라 변동
        alphaPos = dampAlpha(dt, τ=0.1s) = 1 − exp(−dt/τ)  ← dt 에 의존
        카메라 중심이 라이더를 지연 추종하는데 그 지연이 프레임마다 오르내린다
        지도는 카메라 변환으로 그려져 항상 자기 자신과 일치 → 안 흔들린다
        라이더·네임태그는 실좌표에 그려진다 → 상대 위치만 흔들린다
원인 ②  중심 방향과 화면 회전이 다른 값을 썼다
        center 는 원본 bearing 으로 잡고, 지도 회전은 스무딩된 bearing 을 적용
        직선에서는 두 값이 같아 무증상, **코너에서만** 어긋난다
```

**수정(U-10 · `e446913`)** — 계산 순서를 바꿔 bearing 스무딩을 framing 앞으로 옮기고,
`offsetForFraming = normalizeCompass(nextBearing + 180)` 으로 중심 방향을 **지도에 실제
적용되는 bearing** 에서 파생했다. 중심 lerp 계수는 1(지연 0).
**스무딩 상수(τ · max dps)는 하나도 바꾸지 않았다** — 값 조정이 아니라 어느 값을 쓰느냐의 문제였다.

```
비교 스위치  __rtwTick.aligncam(false) → 옛 동작(지연 추종 + 원본 방향 중심)
             기본은 true 이며 배지가 뜨지 않는다. DEV 전용
```

### 7-2. 원인이 아니었던 것 (다시 파지 마라)

세 라운드를 여기에 썼다. 배제 근거가 실측으로 남아 있다.

```
경로 A — LOD·활동 레이어 재동기 100ms      U-2 에서 제거(emit 190→0)했으나 증상 유지
위치 표본 계단                              rAF 연속 적산 — 계단 아님
bearing 계단                                60m/60샘플 평균 — 불연속 없음
zoom 진동                                   전 구간 상수
앱 rAF ↔ Mapbox 렌더 위상 어긋남            lagFrames 677 개 전부 0 · render:write 1:1
라벨 재배치 · map.stop() · GLB 레이어       사용자가 스위치로 하나씩 꺼 본 결과 전부 무관
```

⚠ **교훈** — 「화면이 흔들린다」와 「화면 위 물체가 흔들린다」는 원인이 완전히 다르다.
무엇이 무엇에 대해 움직이는지부터 확정하지 않으면 라운드를 태운다. 여기서 세 번 태웠다.

### 7-3. U 계열에 남은 것

```
U-1R   1 m 구도에서 머리(헬멧 top) 잘림 — 보류. INSTRUCTION-U1R-보류.md
       U-1 의 계측(화면 bbox 높이 0)이 무효였던 것이 미해결의 원인
       전고를 GLB AABB 에서 산출하고 투영을 실제 렌더 행렬로 바꿔야 한다
U-6    구도 미세조정 — 보류. INSTRUCTION-U6-미실행.md
       사용자가 5.5 m 구도를 승인해 급하지 않다
관측    maxZoom 24 클램프 — 프레이밍이 요구한 zoom(최대 25.06)이 적용된 적이 없다
       U-5 에서 실측(getZoom 전 구간 24). U-1R 을 재개하면 함께 풀어야 한다
```
