...
| **S3-DIAG-R1** | 체인 재판정 — 초과량 판정(m 환산) · 전 이벤트 보존 · 구간별 시각 계측 · `endToEndMs` | **배포 (미착수)** |
...
→ **「③→④ 배달 지연이 지배 링크」는 S3-DIAG-R1 이 검증할 가설로 둔다.**
...
분리해 기록하며, 현재 peer 위치 어긋남의 원인으로 지목하지 않는다. S3-DIAG-R1 범위 밖이다.
