...
| Rider-Cycle Fit (결합) | ✅ | 🔶 | 진행 | 별도 rider GLB + 독립 cycle GLB 결합·피팅. **단계 0(register-inputs+extract-glb-meta: AABB·본·프리뷰)·A(register-anchors+extract-anchors: 실측 앵커·다리 정본 430/350)·verify-fit(안장 파생식·위상·ETT 불변식) 완비.** 안장 혼합좌표 교정됨. 다음 우선순위=render-fit.py(승인 전제조건). 단계 B~E·promote 미구현 |
